        ...
        try:
            ...
            if len(cash_flows) <= 1:
                ...
                with col1:
                    ...
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{ytm_anualizada:.4%}</h3>", unsafe_allow_html=True)
                with col2:
                    st.markdown("**TIR Efectiva**")
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{ytm:.4%}</h3>", unsafe_allow_html=True)
                
                # Segunda fila - Duraciones
                col3, col4 = st.columns(2)
                with col3:
                    st.markdown("**Duración Macaulay**")
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{macaulay_duration:.2f} años</h3>", unsafe_allow_html=True)
                with col4:
                    st.markdown("**Duración Modificada**")
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{modified_duration:.2f} años</h3>", unsafe_allow_html=True)
                ...
                with col1:
                    ...
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{capital_residual:.2f}</h3>", unsafe_allow_html=True)
                
                with col2:
                    st.markdown("**Intereses Corridos**")
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{accrued_interest:.4f}</h3>", unsafe_allow_html=True)
                
                with col3:
                    clean_price = bond_price - accrued_interest
                    st.markdown("**Precio Limpio**")
                    st.markdown(f"<h3 style='margin-top: -10px; margin-bottom: 0;'>{clean_price:.2f}</h3>", unsafe_allow_html=True)
